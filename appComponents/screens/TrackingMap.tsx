...
const TrackingMap = () => {
		...
		if (0 === timer) {
			...
			fetchCloseMembers(coordinate, (coordinates: any) => {
				if (Array.isArray(coordinates)) {
					console.log('membersCoordinates',coordinates); // TODO remove this line
	...
	const onPlaceSelected: Function = (id: string | null, closeMemberCoordinate: LatLng | null = null): any => {
		...
		if (null !== id) {
			...
			const geofence = geofences.find((elem: any) => elem.name === pointOfInterest.poiName);
		...
		<SafeAreaView style={styles.root}>
			{building && (
				...
				>
					...
					{0 !== closeMembersCoordinates.length && closeMembersCoordinates.map((closeMemberCoordinate: any, index: number) =>
						<Marker
							key={index}
			...
			<CloseMembersIndicator isCloseMembersLoadingInProgress={isCloseMembersLoadingInProgress} isCloseMembersNotLoaded={isCloseMembersNotLoaded}/>
